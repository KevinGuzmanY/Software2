...
async function createProduct(productData) {
  const product = new Product(productData.name, productData.description, productData.url, productData.tags, productData.userId);
...
async function searchProductsByQuery(filters) {
    try {
        const repProduct = productRepository.searchProductsByQuery(filters);
        return repProduct;
    } catch (error) {
